...
- M2.6.1：原生 External 改用独立 16ms `DispatcherTimer`；不再依赖隐藏 WPF 壳的 `CompositionTarget.Rendering`
- TrackNative 继续作为发现加速层；WPF 仅保留 Child

**验收：** 见 [M2.6 迁移方案](research/M2.6-原生ExternalOverlay迁移.md)；托盘焦点往返手工通过。
**顺序：** Overlay 门禁已过；M3 以非侵入方式接线（见下）。

---

### M3 — 物理赛车可玩原型（✅ 已通过 · 非侵入）

**目标：** 能玩；规则符合调研 §1.2。**硬规则：** 不挖 M2.6.2 承重墙——物理不得进入 `TickExternalFrame`。详见 [M3-物理赛车.md](research/M3-物理赛车.md)。
...
- [x] **死亡**：车质心离开图表可见矩形 → Game Over；翻车不单独判死  
- [x] 重开：空格或托盘；重置到视口左侧缓坡上方  
- [x] 绘制：车 + HUD（原生 Direct2D / Child WPF）；薄契约 `SetCarPose`；**不**自绘第二条山路  
- [x] 捕获与物理解耦：旁路 `DispatcherTimer` RaceHost；高度场经 `HeightFieldUpdated` 出口  
- [x] 手工验收：贴地 / 翻车 / 出界 / 托盘焦点往返仍符合 M2.6.2（2026-08-09）  

**验收：** ✅ **已通过**（2026-08-09；管理员 + Taskmgr 前台）
...
### M4 — 玩家壳 + 降级与发布（可选，5–8 天）

在 M3 可玩后做。**不单插 UX 阶段**：与 lunar-lander 的差距主要在默认 Play、循环与提示，和降级/发布同属「可分享交付」，合入本里程碑。详见 [M4-玩家壳与发布.md](research/M4-玩家壳与发布.md)。

#### A. Play / Debug 分离

- [ ] 默认 **Play**：关拟合线、关调试描边；诊断 HUD（`cap=` / `wda=` / `keys=`）不进玩家画面  
- [ ] 托盘「高级 / 调试」才开橙折线与红框；Tab（或等价）切换 debug  
- [ ] 去掉「有车就强制画 chrome」；玩家 HUD 只用短文案（速度 / 目标 / 提示）  
- [ ] 车与字色重做一版可读配色（非验收绿块 + 大红诊断字）

#### B. 玩法循环与提示（对齐 lunar-lander 量级，不做升级树）

- [ ] 开局提示层：场地就绪后「开始 / 点击或托盘开赛」+ 一行操作（W 加门 / S 减门 / Space 重开）  
- [ ] 极简循环：存活距离或存活时间 + 出界/翻车结算文案；Space / 托盘重开  
- [ ] （可选）燃料或「过热」消耗，用完削弱动力——有则更好，无则用距离即可  
- [ ] 托盘玩家项前置（开赛 / 重开 / 退出）；跟踪后端与跟随方式收入「高级」

#### C. 降级、稳健与发布（原 M4）

- [ ] 捕获连续失败 → PDH 影子山路 + 明确「降级模式」提示  
- [ ] 找不到 `CvChartWindow` → 客户区比例裁剪 fallback  
- [ ] 完整性不匹配（管理员 Taskmgr + 标准用户游戏）→ 检测并提示以管理员重开  
- [ ] 启动自检（探针式）：写 `docs/research/self-check` 日志  
- [ ] 单文件/基础发布、代码签名（若需要）；已知问题写入 README  

**验收：**

| 项 | 标准 |
|---|---|
| Play 默认 | 无橙拟合线、无红诊断框；能看懂怎么开 |
| 循环 | 有目标感（距离/时间）+ 成败文案 + 一键重开 |
| 降级 | 捕获失败有提示且可降级续玩或明确不可玩 |
...
**一句话：** 先钉框（M1），再**加固原生跟踪与 CPU 页判定（M1.5）**，再证蓝线拟合（M2），再上物理车（M3 ✅），可选 **M4 玩家壳 + 降级与发布**（不单插 UX 阶段）。
