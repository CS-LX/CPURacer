...
- [ ] 详读 TaskmgrPlayer `SetParent`/`MoveWindow` 路径，做尖兵对比（外部 Overlay vs 透明子锚点）  
- [ ] 新建 `CPURacer.TrackNative`（vcxproj 进 `CPURacer.sln`），`OutDir`/自动复制满足 §0.4  
- [ ] C# 薄封装替换 M1 `Timer` 轮询主路径  
- [ ] WinEvent：位置 + 前台 + 显隐  
- [ ] CPU 页启发式，单测/手工表  
- [ ] README：唯一构建命令 + M1.5 验收步骤  
