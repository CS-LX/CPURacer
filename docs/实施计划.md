...
- [x] `RaceSim`：Box2D（`Box2D.NetStandard`→Box2DX）世界；地形 = edge chain ← `heightField`（整链重建；分块脏更新后置）  
- [x] 车辆：车身 + 双轮 + 转动关节电机；重力、摩擦；可翻车  
- [x] 卷轴：`plotWidth/60` 近似；车相对图面向右开以免被卷出  
- [x] 输入：`GetAsyncKeyState`（W/↑ 油门、S/↓ 刹车）；翻车后切断有效操控  
...
- [ ] 手工验收：贴地 / 翻车 / 出界 / 托盘焦点往返仍符合 M2.6.2  
