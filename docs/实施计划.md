...
- [ ] 创建 `src/CPURacer.sln` 与上述项目  
- [ ] 托盘菜单：开始/停止跟踪、调试开关占位、退出  
- [ ] 链接调研结论到 README  
...
- [ ] `TaskmgrWatcher`：按进程名找 `Taskmgr.exe`；枚举 `TaskManagerWindow`  
- [ ] 子树收集可见 `CvChartWindow`，按面积选最大者作为 ROI  
- [ ] 可选：检测侧栏多缩略图并存，启发式确认在「性能」页  
- [ ] Overlay：`AllowsTransparency`、Alpha≥1、`Topmost`、`ShowInTaskbar=false`  
- [ ] 每帧/定时更新 `Left/Top/Width/Height`（物理→DIU）  
- [ ] 前台逻辑：Taskmgr 或 Overlay 为前台时显示，否则移出屏幕（参考 `Top=20000`）  
- [ ] 切离性能页 / 找不到大图 → Sleep  
- [ ] 调试：半透明红框描边 ROI（确认对准绘图区而非含大标题的过大矩形）  
